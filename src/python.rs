...
fn cytoscape_node_json(
    ...
    is_highlighted: bool,
    is_orphan: bool,
    all_distances: &HashMap<String, usize>,
) -> String {
    ...
    let orphan_attr = if is_orphan {
        r#", "is_orphan": true"#
    } else {
        r#", "is_orphan": false"#
    };

    // Serialize all_distances as JSON object
    let distances_json = if all_distances.is_empty() {
        String::from("{}")
    } else {
        let mut distances_vec: Vec<_> = all_distances.iter().collect();
        distances_vec.sort_by_key(|(k, _)| *k);
        let distances_str = distances_vec
            .iter()
            .map(|(k, v)| format!(r#""{}": {}"#, escape_json_string(k), v))
            .collect::<Vec<_>>()
            .join(", ");
        format!("{{ {} }}", distances_str)
    };

    format!(
        r#"    {{ "data": {{ "id": "{}", "label": "{}", "type": "{}" {} {}, "all_distances": {} }} }}"#,
...
impl DependencyGraph {
    ...
    /// Compute distances from a single node to all reachable nodes using BFS
    fn bfs_distances_from_node(&self, root_idx: NodeIndex) -> HashMap<ModulePath, usize> {
        ...
        while let Some((node_idx, dist)) = queue.pop_front() {
            let module = &self.graph[node_idx];
            distances.insert(module.clone(), dist);
    ...
    /// Returns a map: module -> (reachable_module -> distance)
    fn compute_all_distances(&self) -> HashMap<ModulePath, HashMap<ModulePath, usize>> {
        let mut all_distances = HashMap::new();

        for root_idx in self.graph.node_indices() {
            let root_module = &self.graph[root_idx];
            let distances = self.bfs_distances_from_node(root_idx);
            all_distances.insert(root_module.clone(), distances);
        }
    ...
    fn to_cytoscape_internal(
        ...
        // Compute all distances for interactive filtering
        let all_distances = self.compute_all_distances();
        ...
        for idx in &nodes {
            ...
            // Get distances from this node to all reachable nodes (as dotted strings)
            let node_distances: HashMap<String, usize> = all_distances
                .get(module)
                .map(|distances_map| {
                    distances_map
                        .iter()
                        .map(|(m, d)| (m.to_dotted(), *d))
                        .collect()
                })
                .unwrap_or_default();
